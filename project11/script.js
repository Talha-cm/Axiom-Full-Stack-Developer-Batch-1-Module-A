...
// Global variables for number of post to fetch per api call and current page
let limit = 5;
let page = 1;
...
function filterPosts(e) {
    ...
    // Process all post in the posts node list
    posts.forEach( post => {
        // Get the title text
        ...
        if ( title.indexOf(filterKeyword) >= 0 || body.indexOf(filterKeyword) >= 0) {
            ...
            // Hide the post if the filterKeyword does not exists in title or body of post
...
// Event Listener 
// Listen for scroll in the brower window
